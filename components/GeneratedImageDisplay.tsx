...
export const GeneratedImageDisplay: React.FC<GeneratedImageDisplayProps> = ({ 
  ...
  const shouldShowSensitiveOverlay = isSensitive && !isSensitiveContentVisible;
        ...
        {isLoading ? (
          ...
          <>
            <img
              src={generatedImage}
              alt="Generated by AI"
              className={`w-full h-full object-contain rounded-lg transition-all duration-500 ${shouldShowSensitiveOverlay ? 'blur-2xl' : 'blur-0'}`}
            />
            
            {shouldShowSensitiveOverlay && (
              <div className="absolute inset-0 z-10 flex flex-col items-center justify-center bg-black/50 rounded-lg text-center">
                <p className="text-white font-semibold text-lg mb-4">Konten Mungkin Sensitif</p>
                <button
                  onClick={onToggleVisibility}
                  className="flex items-center bg-gray-200 text-gray-800 font-bold py-2 px-4 rounded-lg hover:bg-white transition-all"
                  aria-label="Tampilkan konten sensitif"
                >
            ...
            {isSensitive && (
              <button
                onClick={onToggleVisibility}
                className="absolute top-4 right-4 z-20 flex items-center bg-black/50 text-white p-2 rounded-full hover:bg-black/70 backdrop-blur-sm"
                aria-label={isSensitiveContentVisible ? "Sembunyikan konten sensitif" : "Tampilkan konten sensitif"}
              >
                {isSensitiveContentVisible ? <EyeOffIcon className="h-6 w-6" /> : <EyeIcon className="h-6 w-6" />}
              </button>
            )}
